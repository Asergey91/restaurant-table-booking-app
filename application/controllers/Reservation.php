...
class Reservation extends CI_Controller {
	...
	public function make_reservation(){
	  ...
	  //this return an array with the status, error message, and if validation is successfull an array of variables to create a reservation
    ...
    //Converting time to unix (since vlidation library has problems with format when comparing).
    //Checking that the submited time is in the future, last element in the array is the submitted time
    //$time array: 0=now, 1=distant year 2100, 2=submitted time, 3=end of reservation
    $time=[strtotime("now"), 4102448461, strtotime($input['start']), strtotime($input['start'])+(60*60)];
    $day=date('l', $time[2]);
